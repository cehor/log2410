...
void FileChecksumCalculator::visit(const AudioFile & f)
{
	...
	{
		m_checkSum = combineHash(m_checkSum, computeChunkHash(f.getChunkSize(), it->get()), j++);
	}
}
...
void FileChecksumCalculator::visit(const MemAudioFile & f)
{
	...
	{
		m_checkSum = combineHash(m_checkSum, computeChunkHash(chunkSize, buf + chunkSize * i), i);
	}
