...
{
	for (Chunk_const_iterator it = f.begin(); it != f.end(); it++)
	{
		Chunk_iterator FileOut(*m_outFile, m_outFile->getNumberChunks());

		const char* x = it->get();
	    char* y = FileOut->get();
		memcpy(y, x, it->size());

		findReplace(y, y + FileOut->size());

		m_outFile->addChunk(FileOut);
	}
	// A COMPLETER:
	// Iterer sur chaque Chunk du fichier d'entree
	//    - Construire un nouvel iterateur pointant a la fin du fichier de sortie
	//    - Initialiser la memoire de l'iterateur de sortie avec le contenu du Chunk d'entree
