...
void Test_TP4::executeProxyTest()
{
...

	{
		...
		// AJOUT
		// On créé un doublon qui ne sera pas modifié, afin de comparer
		AudioFile compareFileOut("audiof_proxy.bin");

		for (Chunk_iterator iter = memFileIn.begin(); iter != memFileIn.end(); ++iter)
			memFileOut.addChunk(iter);

		/*TEST AJOUTÉ*************************************************************************/
		bool compare = CompareFilesEqual(memFileIn, compareFileOut);
		std::cout << "PROXY Test 1: " << (compare ? "ECHEC" : "SUCCES") << std::endl;
	...
	}
	{
		...
		std::cout << "PROXY Test 2 (AJOUTÉ): " << (compare ? "SUCCES" : "ECHEC") << std::endl;
	}
}
